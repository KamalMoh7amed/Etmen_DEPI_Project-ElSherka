...
namespace Etmen_DAL.Repositories.Implementations
{
    public class ChatMessageRepository : GenericRepository<ChatMessage>, IChatMessageRepository
    {
        ...
        public async Task<IEnumerable<ChatMessage>> GetByConversationAsync(string u1, string u2)
            => await _dbSet.Where(m => (m.SenderId == u1 && m.ReceiverId == u2) || (m.SenderId == u2 && m.ReceiverId == u1)).OrderBy(m => m.SentAt).ToListAsync();
        ...
        public async Task<IEnumerable<ChatMessage>> GetRecentMessagesAsync(string userId, int count)
            => await _dbSet.Where(m => m.SenderId == userId || m.ReceiverId == userId).OrderByDescending(m => m.SentAt).Take(count).ToListAsync();
